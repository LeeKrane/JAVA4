...
public class Main {
	...
	static List<Schueler> schuelerListFromFile;
	static List<String> schuelerStringListFromFile;
	...
	static {
		try (Stream<String> reader = Files.lines(Paths.get("src", "main", "resources", "labor03", filename))) {
			schuelerStringListFromFile = reader.collect(Collectors.toList());
			schuelerListFromFile = schuelerStringListFromFile.stream()
	...
	static List<Schueler> c_csvStrings () {
		...
		List<Schueler> ret = schuelerListFromFile.stream()
				.filter(Schueler::isWeiblich)
	...
	static int e_longestName () {
		...
		int ret = schuelerListFromFile.stream()
	...
	static TreeMap<String, Long> h_getFirstnameMap () {
		...
		TreeMap<String, Long> ret = new TreeMap<>(schuelerListFromFile.stream()
